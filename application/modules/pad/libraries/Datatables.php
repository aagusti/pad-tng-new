...
class Datatables
{
	...
	protected $decimal3_columns = array();
	...
	*/
	...
	public function decimal3_column($columns)
    {
        foreach ($this->explode(',', $columns) as $val) {
            $column                 = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
            $this->decimal3_columns[] = $column;
        }
        return $this;
    }
    ...
    protected function get_paging()
    {
        ...
        $this->db->limit(($iLength != '' && $iLength != '-1') ? $iLength : 10, ($iStart) ? $iStart : 0);
    }
    ...
    protected function get_ordering()
    {
        ...
        for ($i = 0; $i < intval($this->ci->input->get('iSortingCols')); $i++)
            if (isset($mColArray[intval($this->ci->input->get('iSortCol_' . $i))]) && in_array($mColArray[intval($this->ci->input->get('iSortCol_' . $i))], $columns) && $this->ci->input->get('bSortable_' . intval($this->ci->input->get('iSortCol_' . $i))) == 'true')
                $this->db->order_by($mColArray[intval($this->ci->input->get('iSortCol_' . $i))], $this->ci->input->get('sSortDir_' . $i));
    }
        ...
        */
        ...
        if ($sSearch != '') {
            ...
            if (@in_array($mColArray[$i], $date_columns)) {
              ...
              $sWhere .= $this->select[$mColArray[$i]] . "::text = '" . $sSearch . "' OR ";
            } else if (@in_array($mColArray[$i], $decimal3_columns)) {
              $sWhere .= $this->select[$mColArray[$i]] . "::text = '" . $sSearch . "' OR ";
            } else {
    ...
    protected function produce_output($charset)
    {
        ...
        for ($i = 0; $i < sizeof($aaData); $i++) {
			...
			for ($j = 0; $j < sizeof($this->decimal3_columns); $j++) {
                //$aaData[$i][$this->rupiah_columns[$j]] = rupiah($aaData[$i][$this->rupiah_columns[$j]]);
                $aaData[$i][$this->decimal3_columns[$j]] = number_format($aaData[$i][$this->decimal3_columns[$j]], 3, ',', '.');
            }
            ...
            for ($j = 0; $j < sizeof($this->checkbox_columns); $j++) {
                ...
                $aaData[$i][$this->checkbox_columns[$j]] = $aaData[$i][$this->checkbox_columns[$j]] == 1 ? '<i class="fa fa-check text-success"></i>' : '';
            }
        }
        $sOutput = array(
	//'Query' => $qry,
